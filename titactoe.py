...
class TicTacToe:
    ...
    def move(self):  # ход
        def is_move_valid(x, y):
            return 1 <= x <= 3 and 1 <= y <= 3 and self.table[x-1][y-1] == '*' # проверка на возможность хода
        ...
        while not is_coords_valid:
            ...
            if not is_coords_valid: # координаты не соотвествуют размерам игрового поля
                print('Ошибка, вы ввели неправильные координаты!')
        self.table[x-1][y-1] = self.current_move # изменение знаков на игровом поле
    ...
    def reload(self):  # перезагрузка игры
        ...
        if self.winner == '' or self.winner == 'n':
            self.current_move = 'x' # ход крестика
        else:
    ...
    def get_current_player_name(self):  # получение имени текущего игрока
        return self.player_x if self.current_move == 'x' else self.player_0 # возвращаем ход
